...
int main(void){
    ...
    printf("CONSULTA POR ESTADO %s\n", estado_consulta);
    for(int i = 0; i < n; i++){
        if(strcmp(turma[i].local.estado, estado_consulta) == 0){
            total_estado++;
            if(aluno_situacao(&turma[i]) == APROVADO)
                aprovados_estado++;

            printf("%d %s %s %.2f %s\n",
                   turma[i].matricula,
                   turma[i].nome,
                   turma[i].local.cidade,
                   aluno_media(&turma[i]),
                   situacao_texto(aluno_situacao(&turma[i])));
        }
    }
    printf("Total no estado: %d\n", total_estado);
    printf("Aprovados no estado: %d\n", aprovados_estado);
